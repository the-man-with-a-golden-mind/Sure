...
function sure_dom_mount_src() {
  return "var SureDom={mount:function(app){"
    ...
    + "var ev=" + JSON.stringify(["click","input","change","scroll","mousedown","mousemove","mouseup","keydown","keyup","submit","wheel","pointerdown","pointermove","pointerup","focus","blur","touchstart","touchmove","touchend"]) + ";"
    ...
    + "function onEv(e){try{var t=targetOf(e);if(!t||!t.getAttribute)return;var msg=t.getAttribute('data-sure-on-'+e.type);if(msg==null)return;var raw=wireOf(e,msg,t);if(e.type==='scroll'||e.type==='mousemove'){rafRaw=raw;if(!rafId)rafId=requestAnimationFrame(function(){rafId=0;var r=rafRaw;rafRaw=null;if(r){model=app.step(r)(model);draw();}});return;}model=app.step(raw)(model);draw();}catch(_e){}}"
    ...
    + "var raw=wireOf(e,msg,t);if(e.type==='scroll'||e.type==='mousemove'){rafRaw=raw;if(!rafId)rafId=requestAnimationFrame(function(){rafId=0;var r=rafRaw;rafRaw=null;if(r){var p=pairOf(app.step(r)(model));model=p.fst;draw();runCmd(p.snd||'');applySub(app.listen(model));}});return;}var p=pairOf(app.step(raw)(model));model=p.fst;draw();runCmd(p.snd||'');applySub(app.listen(model));}catch(_e){}}"
    + "for(var i=0;i<ev.length;i++)document.addEventListener(ev[i],onEv,true);"
