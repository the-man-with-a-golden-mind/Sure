...
`sure test` runs the bounded prover list, bounded checks, `Main`, `Test.main` (`Test.ci.suite` then `Test.host`), then prove-edge cases. It does not run `Prove.all` or `Test.full`. A failing unit or a false equality exits 1.
