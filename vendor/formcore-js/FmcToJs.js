...
function compile_defs(defs, main, opts) {
  ...
  if (isio) {
    ...
    if (hneed.proc) {
    code += "  var host_unpack = function(s, i) {\n";
    code += "    s = String(s || '');\n";
    code += "    if (i >= s.length) return null;\n";
    code += "    var nl = s.indexOf('\\n', i);\n";
    code += "    if (nl < 0) return null;\n";
    code += "    var n = parseInt(s.slice(i, nl), 10);\n";
    code += "    if (!isFinite(n) || n < 0) return null;\n";
    code += "    var start = nl + 1;\n";
    code += "    if (start + n > s.length) return null;\n";
    code += "    return {value: s.slice(start, start + n), next: start + n};\n";
    code += "  };\n";
    code += "  var host_parse_argv = function(param) {\n";
    code += "    var s = String(param || '');\n";
    code += "    var empty = {file: '', cwd: undefined, env: Object.assign({}, process.env), args: []};\n";
    code += "    var file = host_unpack(s, 0); if (!file) return empty;\n";
    code += "    var cwd = host_unpack(s, file.next); if (!cwd) return empty;\n";
    code += "    var envb = host_unpack(s, cwd.next); if (!envb) return empty;\n";
    code += "    var env = Object.assign({}, process.env);\n";
    code += "    var j = 0; var blob = envb.value;\n";
    code += "    while (true) {\n";
    code += "      var k = host_unpack(blob, j); if (!k) break;\n";
    code += "      var v = host_unpack(blob, k.next); if (!v) break;\n";
    code += "      if (k.value) env[k.value] = v.value;\n";
    code += "      j = v.next;\n";
    code += "    }\n";
    code += "    var args = []; var i = envb.next;\n";
    code += "    while (true) {\n";
    code += "      var a = host_unpack(s, i); if (!a) break;\n";
    code += "      args.push(a.value); i = a.next;\n";
    code += "    }\n";
    code += "    return {file: file.value, cwd: cwd.value || undefined, env: env, args: args};\n";
    code += "  };\n";
    ...
    code += "          try { await run_io(relLib, io.release(resource), 0, relAc); } catch (eRel) {}\n";
    code += "        }\n";
    code += "      default:\n";
